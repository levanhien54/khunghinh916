...
- 🤖 **Chế độ Tự động (bám người nói)** — nút "Phân tích video" chạy ở luồng nền:
  - Phát hiện khuôn mặt CPU **YuNet mặc định** (CNN, MIT, model ~227KB đã bundle ở
    `models/`, thu nhỏ về 320px trước suy luận — chính xác hơn + nhanh hơn Haar);
    tự **fallback Haar cascade** (có sẵn OpenCV) nếu thiếu model. Detection chạy
    **song song thread-pool** (Haar/YuNet nhả GIL) — phân tích nhanh gấp ~3-4×.
...
│   │   ├── yunet_face.py       #   YuNet (MIT, bundle sẵn, downscale 320px — MẶC ĐỊNH)
│   │   ├── haar_face.py        #   Haar cascade có sẵn OpenCV — fallback zero-download
│   │   ├── factory.py          #   tự dùng YuNet bundle, ngược lại fallback Haar
